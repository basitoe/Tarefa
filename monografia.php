    ...
    <div class="panel panel-default panel-table">
        <div class="panel-heading" >
              ...
              <p>
                	...
                	<div class="divH"><label>Tarefas</label></div>
                	<?php 
						...
						<form name="form1" method="post" action="">
				            	...
				            	<table id="myTable" class="table table-condensed  table-bordered table-list table-hover">
					                  <thead style="background: #eee;">
										...
										<th>Observações</th>
										<th>Supervisor</th>
		<?php if ($_SESSION['usuarioNivelAcesso'] != 3) { ?><th>Responsavel</th><?php } ?>
										<th>Acções</th>
									  </tr>
									</thead>
										...
										<?php 
										 $user=$_SESSION['usuarioLogin'];
											while($linhas = mysqli_fetch_array($resultado)){
							                 ...
							                 ?>
												<tr <?php if (!$linhas['url'] && $_SESSION['usuarioNivelAcesso'] != 3) { ?> style="color: none;" <?php } ?>>
											<?php
												...
												<?php if ($linhas['url'] && ($_SESSION['usuarioNivelAcesso'] == 1 || $_SESSION['usuarioNivelAcesso'] == 5)) {?><a href='#avaliar_Modal' data-toggle="modal" data-whatever="<?php echo $linhas['id'];?>" data-whatevertema="<?php echo $linhas['tema'];?>" data-whateverdescricao="<?php echo $linhas['descricao'];?>"><?php } ?><button type='button' name='Activar' class='btn btn-sm btn-warning btn-block' title=""><span class=''>Não avaliado</span> </button></a></td>
													...
													<td class="text-center"><a href='#'><button type='button' name='Activar' class='btn btn-sm btn-success btn-block ' title=""><span class=''>Registado</span> </button></a></td>
													<?php
													}
													echo ( "<td>".$linhas['obs']."</td>");
							                        echo ( "<td>".$linhas['idSupervisor']."</td>");
...
<!-- /container -->
				      ...
				      <div class="modal-content">
				          ...
				          <div class="modal-body">   
				            ...
				            <div class="form-group">
				              ...
				              <textarea  id="descricao" name="descricao" placeholder="Descrição" class="form-control" ></textarea>
				            </div>
				      ...
				      <div class="modal-content">
				          ...
				          <div class="modal-body">   
				            ...
				            <div class="form-group">
				              ...
				              <input type="text" readonly="" id="tema" name="tema" autofocus="" maxlength="750" class="form-control" placeholder="Digite o Nome da Empresa" required>
				              <label>Descricão(*)</label>
				            ...
				            <div class="form-group">
				              ...
				              <select required="" name="estado"  class="form-control">
				              		...
				              		<option value="3">Registado</option>
				              </select>
				            </div>
				            <div class="form-group">
			...
			if(isset($_POST['aprovar_proj_Modal'] )){
				...
				if ($estado==2) {
					...
					$nome_estado="Registado";
				}

				$obs = $_POST["obs"];
				$query=mysqli_query($conectar,"UPDATE `monografias` SET `estado`=$estado,`obs`='$obs', idUserAprov='$idUsuario' WHERE id=$id");
				$notificacao=mysqli_query($conectar,"INSERT INTO `notificacoes`(`Titulo`, `Texto`, `id_nivel_acesso`, `id_Usuario`, `id_Destinatario`, `Estado`) VALUES ('A sua Requisição foi $nome_estado!','A sua Requisição foi $nome_estado!','','$idUsuario','$estudante_usuario','1')");
...
<!-- Inicio Modal Apagar -->
				...
				<h4 class="modal-title custom_align" id="Heading">Apagar Registo</h4>
				</div>
				<br>
				<div class="text-center"><i class="fa fa-file fa-7x fa-fw" style="color: #1EA4DD;"></i></div>
...
<!-- Inicio Modal Editar -->
 <div id="editar_proj_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
          <div class="modal-header">            
            <h4 class="modal-title">Editar dados da Empresa</h4>
            <button type="button"  class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
          </div>
          <br>
         <div class="text-center"><i class="fa fa-file fa-7x fa-fw" style="color: #1EA4DD;"></i></div>
...
if(isset($_POST['Save'] )){
 ...
 <div id="carregar_mono_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
          <div class="modal-header">            
            <h4 class="modal-title">Carregar Documentos da Empresa</h4>
            <button type="button"  class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
          </div>
          <br>
         <div class="text-center"><i class="fa fa-file fa-7x fa-fw" style="color: #1EA4DD;"></i></div>
